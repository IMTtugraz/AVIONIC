function [b1,u,ind,par] = coil_construction_opt(kdata,mask,par_in);
	...
	[u0,crec] = get_sos_crec_opt(kdata,mask);
	...
	for j=2:ncoils
		...
		%Get new b1
		phs = b1_grad_opt(w.*absb1(:,:,ind(j)).*u(:,:,j-1),w.*crec(:,:,ind(j)),{'stop_par',500;'mu',b1_reg});
